...
```{r}
plot_correlation(na.omit(df1_clean), maxcat = 5L, type = "d")
```
...
2. Gender (0: female, or 1: male) - Gender

* Standard to include within medical modelling.

3. SAPS-I score - SAPS1
...
5. Albumin min - Albumin_min

*no strong correlations between other predictor variables, other than other ablumin variables eg. Albumin_max - which we will not include in the analysis.

6. Serum creatinine (mg/dL) - Creatinine_max
...
7. Blood urea nitrogen (mg/dL) - BUN min

* Predictor of kindey impairment

8. Glasgow Comas Score - GCS_max
...
9. Troponin I - TroponinI_max

* Predictor of cardiovascular impairment, Troponin in released from the heart myocardium and is indicative of damage to the heart.

10. Troponin T - TroponinT_max
...
13. Arterial O2 Concentration (%) - PaO2_max

* Oxygen Concentration in blood vessels transporting oxygenated blood to the body.
* Negatively correlated with in_hospital_death.

14. Arterial CO2 Concentration (%) - PaCO2_diff

15. Platelets - Platelets_max

* 

16. Serum creatinine (mg/dL) - Creatinine_max

* High levels (above 250) are a clincial indicator of renal failure, which is a very serious complication somtimes requiring dialysis.
* strongly Correlated with BUN but also weakly correlated with in_hospital_death.

17. Infection Indicator - WBC_min
...
```{r}
arr_delay_df2 <- icu_patients_df1[, c("in_hospital_death", "Age", "Gender", "SAPS1", "SOFA", "Albumin_min", "Creatinine_max", "BUN_min", "GCS_max", "TroponinI_max", "TroponinT_max", "MAP_max", "MAP_min", "PaO2_max", "PaCO2_diff", "Platelets_max", "Creatinine_max", "WBC_min")]
...
categorical_vars <- c("Albumin_min", "Bilirubin_max", "Creatinine_max", "HCO3_diff", "SAPS1", "BUN_min", "GCS_max", "pH_min", "Glucose_max", "SaO2_max", "Height", "Weight_diff")
...
```{r}
#put column names into a variable 
categorical_vars <- paste(c("Albumin_min", "Bilirubin_max", "Creatinine_max", "HCO3_diff", "SAPS1", "BUN_min", "GCS_max", "pH_min", "Glucose_max", "Height", "Weight_diff"), collapse = " + ")
...
```
_The global test for proportional hazards is significant and that this is largely due to the three predictor variables (ICU_type, SOFA and GCS_max)._
