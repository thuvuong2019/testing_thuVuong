...
glm_analysis_df <- na.omit(glm_analysis_df)
```
...
_likewise with continuous variables are mostly un correlated, however, Creatinein_max/BUN_min, and SOFA/SAPS1 correlations raise some concerns. After the Univariate model fitting we can select the variable with a higher statistical significance from these pairs._
...
for(i in 1:(length(histogram_data)-1)) {
...
```{r}
probabilities <- predict(reduced_multivariate_mod, type=c("response"))
glm_analysis_df$probabilities <- probabilities

library(pROC)
g <- roc(in_hospital_death ~ probabilities, data = glm_analysis_df)
print(g) #print AUC
plot(g)  #plot RoC
```
...
###Fitting our final model to the Unimputed data
...
###determine unimputed odds ratios

```{r}
unimputed_betas <- coef(unimputed_multivariate_mod)

exp(unimputed_betas)

exp(cbind(OR = unimputed_betas, confint(unimputed_multivariate_mod)))
```
...
_Our final model was contained variables "Age", "Gender", "ICUType", "SAPS1", "SOFA", "Creatinine_max", "BUN_min", "GCS_max", and "PaO2_max", of these variables all were found to be sgnificant when fitted to both the imputed and unimputed datasets with the exception of "Gender". Although not statistically significant, "Gender" was retained within the final model because of it's clinical significance within medical modelling. In both imputed and unimputed models ICUTypeCardiacSurgery Recovery Unit was found to have the highest odds risk._
